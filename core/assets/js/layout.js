...
window.onscroll = () => {
    console.log('0 onscroll', onscroll)
  if (startFlag) {
    console.log('1 startFlag', startFlag)
    const scrollDown = this.scrollY >= initialScroll
    const scrollLimit = qty >= 1 && qty <= sectionsQty
    // Verify that the scroll does not exceed the number of sections
    if (scrollLimit) {
        console.log('2 scrollLimit', scrollLimit)
      body.style.overflowY = 'hidden' // Lock el scroll
      if (scrollDown && qty < sectionsQty) {
        console.log('3 scrollDown && qty < sectionsQty')
       ...
       console.log('4 !scrollDown && qty > 1')
      ...
      active.style.top = (52 + 30) * (qty - 1) + 'px'
    }
    console.log('SLIDE', qty)
